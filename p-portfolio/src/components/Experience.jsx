...
const Experience = () => {
    return (
        <div className="flex flex-col gap-12 sm:gap-16 relative">
             ...
             <div className="absolute left-[20px] sm:left-[28px] top-6 bottom-6 w-px bg-zinc-200 dark:bg-zinc-800"></div>
            ...
            {experiences.map((exp, index) => (
                ...
                >
                    {/* Icon Node */}
                    <div className="relative z-10 flex-shrink-0 w-10 h-10 sm:w-14 sm:h-14 rounded-full bg-white dark:bg-black border border-zinc-200/50 dark:border-white/10 flex items-center justify-center overflow-hidden transition-all duration-300 group-hover:border-zinc-300 dark:group-hover:border-white/30 group-hover:scale-105 shadow-[0_2px_10px_rgb(0,0,0,0.02)]">
                        {exp.logo ? (
                            <img src={exp.logo} alt={exp.company} className="w-full h-full object-contain p-1 rounded-full bg-white object-center" />
                        ) : (
                    ...
                    <div className="flex flex-col flex-grow pt-1.5 sm:pt-2">
                        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-1 sm:gap-3 mb-2">
                             <div className="flex flex-col">
                                <h4 className="text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white tracking-tight">{exp.role}</h4>
                                {exp.client && (
                             ...
                             <span className="text-[10px] sm:text-xs font-mono font-bold tracking-[0.2em] uppercase text-zinc-400 dark:text-zinc-500 xl:whitespace-nowrap bg-zinc-100 dark:bg-white/5 w-fit px-3 py-1 rounded-full border border-black/5 dark:border-white/5 mt-2 xl:mt-0">
                                 {exp.period}
                             </span>
                        </div>
                        
                        {exp.url ? (
                            <a href={exp.url} target="_blank" rel="noopener noreferrer" className="text-sm sm:text-base text-zinc-600 dark:text-zinc-400 font-medium hover:text-black dark:hover:text-white hover:translate-x-1 transition-all w-fit cursor-pointer">
                                {exp.company}
                            </a>
                        ) : (
